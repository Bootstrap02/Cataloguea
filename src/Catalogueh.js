...
// //     // 2. Big game stakes: bigDeficit / odd (no -1)
...
// //           setprivateTotal(0);
// //         } else{
// //           setprivateTotal((prev) => {prev - bigShadow})
// //         }
// //       }
...
// //           setPendingStakes((prev) => ({ ...prev, [type]: 0 }));
// //         } else{
// //           setBigDeficit((prev) => {prev - smallShadow})
// //           setPrivateDeficits((prev) => ({ ...prev, [type]: 0 }));
// //           setPendingStakes((prev) => ({ ...prev, [type]: 0 }));
// //         }
// //       }
...
// //   const privateTotal = smallGameKeys.reduce((sum, key) => sum + (privateDeficits[key] || 0), 0);
...
// //             <button onClick={handleReload} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 rounded-xl font-medium text-sm">
...
// //               <input value={inputA} onChange={(e) => setInputA(e.target.value)} placeholder="Home" className="w-28 px-4 py-2.5 border border-red-600 bg-transparent rounded-2xl text-center text-sm" />
// //               <span className="self-center text-2xl text-red-500 font-black">VS</span>
// //               <input value={inputB} onChange={(e) => setInputB(e.target.value)} placeholder="Away" className="w-28 px-4 py-2.5 border border-red-600 bg-transparent rounded-2xl text-center text-sm" />
// //             </div>

// //             <div className="flex justify-center gap-3 mb-8">
// //               <button onClick={handleLoadGame} disabled={isGameLoaded} className={`px-8 py-3 text-white font-bold rounded-2xl transition text-sm ${isGameLoaded ? "bg-gray-600" : "bg-red-600 hover:bg-red-700"}`}>
// //                 LOAD GAME
// //               </button>
// //               <button onClick={handleNextGame} className="px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-2xl text-sm transition">
// //                 NEXT GAME
// //               </button>
// //             </div>
...
// //               {allKeys.map((key) => (
// //                 <button key={key} onClick={() => handleWin(key)} disabled={!fixture || pendingStakes[key] === 0 || isButtonPressed(key)}
// //                   className={`py-4 rounded-2xl font-medium text-xs transition ${isButtonPressed(key) ? "bg-yellow-500 text-black" : key === "winner" ? "bg-yellow-400 text-black" : "bg-blue-600 hover:bg-blue-500 text-white"} ${pendingStakes[key] === 0 || isButtonPressed(key) ? "opacity-50" : ""}`}>
// //                   {labels[key]}<br />
...
// //           {allKeys.map((key) => (
// //             <button key={key} onClick={() => handleWin(key)} disabled={!fixture || pendingStakes[key] === 0 || isButtonPressed(key)}
// //               className={`py-5 rounded-2xl font-medium text-sm transition ${isButtonPressed(key) ? "bg-yellow-500 text-black" : key === "winner" ? "bg-yellow-400 text-black" : "bg-blue-600 text-white"} ${pendingStakes[key] === 0 || isButtonPressed(key) ? "opacity-50" : ""}`}>
// //               {labels[key]}<br />
...
//     oneX: 0, twoX: 0, zeroGoals: 0, sixGoals: 0, ht21: 0,
//   });
//   const [privateTotal, setPrivateTotal] = useState(0);   // ← Now a real state
...
//     // Big stakes: bigDeficit / odd (no -1)
//     bigGameKeys.forEach((key) => {
//       const odd = found[key] || 0;
//       let stake = odd > 1.01 ? Math.round(newBig / odd) : 0;
//       newPending[key] = Math.max(stake, 10);
...
//     // Big stakes total → smallDeficit
//     const bigStakesTotal = bigGameKeys.reduce((sum, key) => sum + (newPending[key] || 0), 0);
//     const newSmall = smallDeficit + bigStakesTotal;
//     setSmallDeficit(newSmall);
//     setSmallShadow(newSmall);

//     // Small stakes: newSmall / (odd - 1)  ← martingale
//     smallGameKeys.forEach((key) => {
//       const odd = found[key] || 0;
//       let stake = odd > 1.01 ? Math.round(newSmall / (odd - 1)) : 0;
//       newPending[key] = Math.max(stake, 10);
...
//     // Update privateTotal state
//     const newPrivateTotal = smallGameKeys.reduce((sum, key) => 
...
//             <div className="flex justify-center gap-3 mb-8">
//               <button onClick={handleLoadGame} disabled={isGameLoaded} className={`px-8 py-3 text-white font-bold rounded-2xl transition text-sm ${isGameLoaded ? "bg-gray-600" : "bg-red-600 hover:bg-red-700"}`}>LOAD GAME</button>
//               <button onClick={handleNextGame} className="px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-2xl text-sm transition">NEXT GAME</button>
//             </div>
...
//               <div>Big Def: <strong className="text-red-400">{bigDeficit}</strong> (S: {bigShadow})</div>
//               <div>Small Def: <strong className="text-purple-400">{smallDeficit}</strong> (S: {smallShadow})</div>
//               <div>Private Total: <strong className="text-orange-400">{privateTotal}</strong></div>
...
//                 isButtonPressed(key) ? "bg-yellow-500 text-black" : key === "winner" ? "bg-yellow-400 text-black" : "bg-blue-600 text-white"
//               } ${pendingStakes[key] === 0 || isButtonPressed(key) ? "opacity-50" : ""}`}
//             >
//               {labels[key]}<br />
//               <span className="text-xs">({pendingStakes[key] || "–"})</span>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 rounded-xl font-medium text-sm"
            >
              <FiRefreshCw className="w-4 h-4" /> Reload
            </button>
          </div>
